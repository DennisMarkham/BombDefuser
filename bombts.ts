...
console.log(beepPreference);
 ...
 function loadModal(): void{
    ...
    var everBeep: boolean = true;

    var beepPreference: boolean|string = localStorage.getItem("everBeep");
    ...
    var randomWire: number = Math.floor(Math.random() * 3) + 1;

    console.log(randomWire);

    var counter: number = 59;
...
$("p").click(function(): void{
    ...
    function timeConverter(t): string{
      var minutes: number|string = Math.floor(t / 60);
    var seconds: number|string = t - (minutes * 60);
    ...
    function time(): void{
     ...
     var counterConverted = timeConverter(counter);
      $("#time").text(counterConverted);
       setTimeout(time, 1000 * 1);
    }

    function bombTimer():void {
      if (alreadyWon == false)
      {
